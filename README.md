...
Transport-agnostic protocol library for the Hi-Link HLK-LD2415H K-band velocity radar sensor ([datasheet](HLK-LD2415H(english).pdf)).  The sensor utilizes millimeter wave radar to measure the velocity of objects within it's field of view.  The sensor speaks a simple line-based serial protocol. This library implements the full protocol engine without any external dependencies. It runs anywhere C++17 does.
...
|---|---|
| Brand | Hi-Link |
| Device | 24G Microwave Vehicle Speed Testing Radar Module |
...
Tests run on the host (no ESP needed): they feed canned protocol bytes through a fake transport and assert on emitted command frames and parsed state.
